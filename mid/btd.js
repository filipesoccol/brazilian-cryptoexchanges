...
let ENDPOINT_API       = 'https://api.bitcointrade.com.br/v1/public';
let ENDPOINT_TRADE_API = 'https://api.bitcointrade.com.br/v1';
...
module.exports = {
    ...
    setOrderbookListener : function (pairs,callback) {
        setInterval(function(){
            ...
            Object.keys(pairs).forEach(function(pair){
                promises.push(module.exports.getOrderbook(pairs[pair].alias));
            });
            q.all(promises)
            .then(function(res){
                callback(res);
    ...
    getOrderbook : function (pair) {
        ...
        return new Promise( (resolve, reject) => {
            publicRequest('/BTC/orders', undefined ,function(result){
                try{

                    if (!result.data){
                        reject('ERROR GETTING TRADES BTD'+ JSON.stringify(result))
                        return;
                    }
    ...
    setTimeout( () => {
        ...
        .end(function (response) {
            ...
            if (typeof response.body === Object)
                error('BTD Error on request ===> '+ method + ' >>> '+ JSON.stringify(response));
